            ...
            <div className="grid grid-cols-2 gap-4 flex-1 overflow-y-auto pb-4">
                {isGenerating ? (
                    ...
                    images.map((img) => (
                        <div
                            key={img.id}
                            className={`group relative aspect-square rounded-xl overflow-hidden border-2 transition-all cursor-zoom-in ${img.selected ? 'border-indigo-600 shadow-lg ring-2 ring-indigo-100' : 'border-transparent hover:border-gray-200'
                                }`}
                            ...
                            <div className={`absolute inset-0 bg-black/40 transition-opacity flex flex-col justify-end p-3 ${img.selected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                                <div className="flex justify-between items-end">
                                    ...
                                    <button
                                        ...
                                        className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${img.selected
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                setPromptImage(img);
                                                // GA Event: View Prompt from thumbnail
                                                if (typeof window !== 'undefined' && (window as any).gtag) {
                                                    (window as any).gtag('event', 'view_prompt', {
                                                        event_category: 'engagement',
                                                        event_label: 'thumbnail_hover',
                                                        image_id: img.id
                                                    });
                                                }
                                            }}
                                        ...
                                        <button
                                            onClick={(e) => {
                                                ...
                                                // GA Event: Download from thumbnail
                                                if (typeof window !== 'undefined' && (window as any).gtag) {
                        ...
                        <div className="mt-6 flex gap-4">
                            <button
                                onClick={() => {
                                    ...
                                    // GA Event: View Prompt from preview modal
                                    if (typeof window !== 'undefined' && (window as any).gtag) {
                                        (window as any).gtag('event', 'view_prompt', {
                                            event_category: 'engagement',
                            ...
                            <button
                                onClick={() => {
                                    onDownload(previewImage.url, previewImage.id);
                                    // GA Event: Download from preview modal
                                    if (typeof window !== 'undefined' && (window as any).gtag) {
                                        (window as any).gtag('event', 'download_image', {
                                            event_category: 'engagement',
                                            event_label: 'preview_modal',
